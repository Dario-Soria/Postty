...
Responde SOLO con las instrucciones (sin explicaciones adicionales):"""
    ...
    def _generate_smart_text_suggestions(self, text_elements: list) -> dict:
        """
        =======================================================================
        SMART TEXT SUGGESTIONS - VERSION 2
        =======================================================================
        Generate suggestions using Gemini's knowledge base.
        No web search required - uses LLM's training data.
        
        Returns dict with suggestions like {"headline": "...", "subheadline": "..."}
        or None if generation fails (caller should fall back to generic suggestions).
        =======================================================================
        """
        ...
        # Determine what info we need based on text_elements
        ...
        print(f"[DEBUG] Smart suggestions V2: product={product_name}, types={elem_types}, has_percentages={has_percentages}, has_ingredients={has_ingredients}")
        
        # Build contextual prompt based on what type of text the reference has
        context_hints = []
        if has_percentages:
...
Para el producto "{product_name}", genera sugerencias de texto REALES y CORTAS para un anuncio de Instagram.

Tipos de texto necesarios: {elem_types}
...
- Si no conoces el producto exacto, genera texto profesional basado en la categoria del producto
- Textos MUY CORTOS (maximo 5-7 palabras por elemento)
- NO uses corchetes, placeholders, ni ejemplos genericos
- Responde SOLO en español
- El headline debe ser el nombre del producto o su beneficio principal
- El subheadline debe ser una caracteristica o ingrediente destacado

Responde SOLO con JSON valido, sin explicaciones adicionales:
{{"headline": "texto real", "subheadline": "texto real", "benefits": "texto real"}}
"""
        ...
        try:
            ...
            result = response.text.strip()
            print(f"[DEBUG] LLM smart suggestions V2 response: {result[:200]}")
            
            # Parse JSON response
            if '{' in result:
                json_str = result[result.find('{'):result.rfind('}')+1]
                suggestions = json.loads(json_str)
                print(f"[DEBUG] Parsed smart suggestions V2: {suggestions}")
                return suggestions
        except Exception as e:
