...
Be specific with the product name if you can see it."""
    ...
    def _build_nanobanana_prompt(self) -> str:
        """
        =======================================================================
        BUILD NANOBANANA PROMPT - VERSION 1 (STABLE)
        =======================================================================
        ...
        - Uses self.design_changes for requested changes
        - Uses self.text_content for text overlay (headline, subheadline, cta)
        - Includes IMMUTABLE RULES for quality control
        ...
        has_changes = self.design_changes and self.design_changes.strip()
        if has_changes:
            # Check if user wants to keep everything the same
            changes_lower = self.design_changes.lower()
            keep_same_keywords = ['mantener', 'igual', 'mismo', 'keep', 'same', 'sin cambios', 'no cambios', 'nada', 'todo bien']
            if any(keyword in changes_lower for keyword in keep_same_keywords):
                changes_section = "- None. Keep reference exact."
            else:
                # Use the user's actual change request
                changes_section = f"- {self.design_changes}"
        else:
            changes_section = "- None. Keep reference exact."
...
2. DO NOT invent elements not present in the reference
3. DO NOT modify anything the user did not request
4. NO spelling errors in text
5. Product must look professional and be the protagonist
6. Commercial advertising quality
7. If there is text, it must be 100% legible
8. Maintain total visual coherence with the reference"""
    ...
    def _build_nanobanana_edit_prompt(self, user_feedback: str) -> str:
        """
        =======================================================================
        BUILD NANOBANANA EDIT PROMPT - VERSION 1 (STABLE)
        =======================================================================
        DO NOT EDIT without explicit permission from the user.
...
Tu respuesta (SOLO el resumen, sin comillas ni explicaciones):"""
    ...
    def _handle_edit_pipeline(self) -> Dict[str, Any]:
        """
        ================================================================================
        VERSION 1 (STABLE) - Edit Pipeline Handler
        DO NOT MODIFY without explicit permission from the user.
        ================================================================================
        ...
        if not self.selected_reference:
            return {"type": "text", "text": "No tengo la referencia guardada. ¿Querés empezar de nuevo?"}
    ...
    def _handle_search_references_by_type(self, post_type: str) -> Dict[str, Any]:
        """
        =======================================================================
        SEARCH REFERENCES BY TYPE - VERSION 3 (STABLE)
        =======================================================================
        ⚠️  DO NOT EDIT without explicit permission from the user.
    ...
    def _summarize_user_changes(self, user_message: str) -> str:
        ...
        Returns a natural language summary of what changes will be applied
        """
        try:
            # Use Gemini to interpret the user's changes contextually
            prompt = f"""El usuario ha indicado los siguientes cambios para un diseño de post:

Mensaje del usuario: "{user_message}"

Tu tarea es resumir brevemente (1-2 oraciones) lo que el usuario quiere hacer. 
- Si quiere mantener todo igual, di "mantener el diseño actual"
- Si quiere cambios específicos, menciona SOLO esos cambios de forma concisa
- Usa lenguaje natural y amigable en español
- NO agregues información que el usuario no haya mencionado
- NO hagas preguntas, solo resume

Responde SOLO con el resumen, sin comillas ni explicaciones adicionales."""
    ...
    def _handle_design_changes(self, user_message: str) -> Dict[str, Any]:
        ...
        if has_text:
            ...
            # Build the response with both detected text and suggestions
            detected_text_str = "\n".join(text_elements_info) if text_elements_info else "Texto detectado"
            suggestions_str = "\n".join(text_suggestions) if text_suggestions else "Sin sugerencias específicas"
            
            text = f"{confirmation_prefix}\n\nVeo que tu referencia tiene los siguientes textos:\n{detected_text_str}\n\n**Mis sugerencias para tu producto:**\n{suggestions_str}\n\n¿Qué textos te gustaría usar? Puedes decirme exactamente qué quieres o aceptar mis sugerencias."
