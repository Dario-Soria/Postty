...
Responde SOLO con las instrucciones (sin explicaciones adicionales):"""
    ...
    # =========================================================================
    # SMART TEXT SUGGESTIONS - VERSION 1 (NEW)
    # =========================================================================
    # Uses Google Custom Search API to find real product information
    # for generating contextual text suggestions instead of placeholders
    # =========================================================================
    
    def _search_product_info(self, query: str) -> str:
        """
        Search for real product information using Google Custom Search API.
        Returns relevant snippets about the product.
        
        Requires GOOGLE_SEARCH_ENGINE_ID environment variable (cx).
        Uses existing GEMINI_API_KEY for authentication.
        """
        import os
        ...
        if not cx:
            print("[DEBUG] GOOGLE_SEARCH_ENGINE_ID not configured, skipping web search")
            return ""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[DEBUG] Web search returned {len(snippets)} results, {len(combined)} chars")
                return combined
            else:
                print(f"[DEBUG] Web search failed with status {response.status_code}")
        except Exception as e:
    ...
    def _generate_smart_text_suggestions(self, text_elements: list) -> dict:
        """
        Generate intelligent text suggestions using web search and LLM.
        
        Analyzes the text_elements from the reference to understand what type
        of information is needed, then searches for real product data.
        
        Returns dict with suggestions like {"headline": "...", "subheadline": "..."}
        or None if search fails (caller should fall back to generic suggestions).
        """
        ...
        # Determine what types of info we need based on text_elements
        needs_benefits = any(
            e.get('type') in ['benefits', 'subheadline', 'description'] 
            for e in text_elements
        )
        needs_ingredients = any(
            'ingrediente' in e.get('detected_text', '').lower() or 
            e.get('type') == 'ingredients' or
            e.get('estimated_purpose') == 'ingredients'
            for e in text_elements
        )
        needs_stats = any(
            '%' in e.get('detected_text', '') or 
            e.get('type') == 'promotion' or
            'resultados' in e.get('detected_text', '').lower()
            for e in text_elements
        )
        
        print(f"[DEBUG] Smart suggestions analysis: benefits={needs_benefits}, ingredients={needs_ingredients}, stats={needs_stats}")
        
        # Search for relevant info based on what the reference shows
        search_context = ""
        
        if needs_benefits:
            search_context += self._search_product_info(f"{product_name} beneficios principales características")
        
        if needs_ingredients:
            search_context += " " + self._search_product_info(f"{product_name} ingredientes activos composición")
        
        if needs_stats:
            search_context += " " + self._search_product_info(f"{product_name} resultados estudios clínicos eficacia porcentaje")
        
        # If no specific needs detected, do a general search
        if not search_context.strip():
            search_context = self._search_product_info(f"{product_name} producto características beneficios")
        
        if not search_context.strip():
            print("[DEBUG] No search results, falling back to generic suggestions")
            return None
        
        # Use LLM to generate contextual suggestions from search results
        elem_types = [e.get('type', 'texto') for e in text_elements]
        
        prompt = f"""Eres un experto en marketing de productos. 
Basándote en esta información REAL del producto "{product_name}":

{search_context[:2000]}

Genera sugerencias de texto CORTAS Y REALES para un anuncio de Instagram.
Los tipos de texto que necesito son: {elem_types}

REGLAS:
- Usa SOLO información real del producto (no inventes datos)
- Textos CORTOS (máximo 5-7 palabras por elemento)
- Si no hay info para un campo, déjalo vacío
- NO uses corchetes ni placeholders

Responde SOLO con JSON válido, sin explicaciones:
{{"headline": "nombre o beneficio principal", "subheadline": "característica o ingrediente destacado", "benefits": "beneficio específico"}}
"""
        ...
        try:
            ...
            result = response.text.strip()
            print(f"[DEBUG] LLM smart suggestions response: {result[:200]}")
            
            # Parse JSON response
            if '{' in result:
                json_str = result[result.find('{'):result.rfind('}')+1]
                suggestions = json.loads(json_str)
                print(f"[DEBUG] Parsed smart suggestions: {suggestions}")
                return suggestions
        except Exception as e:
    ...
    def _build_nanobanana_prompt(self) -> str:
        """
        =======================================================================
        BUILD NANOBANANA PROMPT - VERSION 4 (STABLE)
        =======================================================================
        DO NOT EDIT without explicit permission from the user.
        ...
        if has_text:
            ...
            text_section = f"""Include the following text maintaining EXACTLY the same typographic style, color, size and position as the reference:
{confirmed_text}
- Position: Same as reference
- Style: Same as reference (font, color, effects)"""
        else:
...
10. When in doubt: REMOVE the reference product, KEEP everything else"""
    ...
    def _build_nanobanana_edit_prompt(self, user_feedback: str) -> str:
        """
        =======================================================================
        BUILD NANOBANANA EDIT PROMPT - VERSION 2 (STABLE)
        =======================================================================
        DO NOT EDIT without explicit permission from the user.
...
Tu respuesta (SOLO la acción, sin comillas ni explicaciones):"""
        ...
        if has_text:
            ...
            if self.text_analysis and self.text_analysis.get('text_elements'):
                ...
                for elem in self.text_analysis['text_elements']:
                    ...
                    # Generate suggestion - use smart if available, otherwise generic fallback
                    if using_smart:
                        # Map element types to smart suggestion keys
                        if elem_type in ['headline', 'product-name']:
                            ...
                            # Try subheadline first, then benefits
                        ...
                        elif elem_type == 'brand-name':
                            text_suggestions.append(f"**{friendly_type}:** [Tu marca]")
                        elif elem_type == 'cta':
                            text_suggestions.append(f"**{friendly_type}:** Descubrí más")
                        elif elem_type == 'promotion':
                            text_suggestions.append(f"**{friendly_type}:** [Tu promoción]")
                        elif elem_type == 'price':
                            text_suggestions.append(f"**{friendly_type}:** [Tu precio]")
                        elif elem_type == 'footer':
                            text_suggestions.append(f"**{friendly_type}:** [Tu contacto]")
                        else:
                            text_suggestions.append(f"**{friendly_type}:** [Tu texto]")
                    else:
                        # Fallback to generic suggestions (original behavior)
                        if elem_type in ['headline', 'product-name']:
                            ...
                            text_suggestions.append(f"**{friendly_type}:** [Ej: Compra ahora, Descubre más]")
                        elif elem_type == 'promotion':
                            text_suggestions.append(f"**{friendly_type}:** [Tu promoción]")
                        elif elem_type == 'price':
                            text_suggestions.append(f"**{friendly_type}:** [Tu precio]")
                        elif elem_type == 'footer':
                            text_suggestions.append(f"**{friendly_type}:** [Tu contacto o info adicional]")
                        else:
                            text_suggestions.append(f"**{friendly_type}:** [Tu texto]")
    ...
    def _handle_text_confirmed(self, user_message: str) -> Dict[str, Any]:
        ...
        if any(kw in user_message.lower() for kw in accept_keywords):
            ...
            # Use suggested text (would need to be stored)
            pass
        else:
            # Parse custom text
            self.text_content = self._parse_text_content(user_message)
