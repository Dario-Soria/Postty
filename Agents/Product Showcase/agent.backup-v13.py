...
"""
        ...
        try:
            ...
            result = response.text.strip()
            # V3: Use stderr to avoid backend interpreting this as agent response
            import sys
            print(f"[DEBUG] LLM smart suggestions V3 response: {result[:300]}", file=sys.stderr)
            ...
            if '{' in result:
                ...
                # V3: Use stderr to avoid backend interpreting this as agent response
                print(f"[DEBUG] Parsed smart suggestions V3: {filtered}", file=sys.stderr)
    ...
    def _build_nanobanana_prompt(self) -> str:
        ...
        # === TEXT section ===
        # V3: Support both indexed keys (headline_1) and non-indexed (headline)
        has_text = self.text_content is not None and len(self.text_content) > 0
        if has_text:
            text_parts = []
            
            # Collect all headlines (indexed or not)
            for i in range(1, 10):
...
1. ONLY use the text listed above - DO NOT copy any text from the reference image
2. If the reference has text like percentages, taglines, or slogans that are NOT in the list above, DO NOT include them
3. The text must look like it was always part of the original design
4. DO NOT use a generic font - analyze and match the reference typography exactly
5. Replace ALL reference text with ONLY the text provided above"""
        else:
        ...
        try:
            ...
            with open(product_image, 'rb') as product_file:
                ...
                if has_text:
                    ...
                    # V3: Convert text_content dict to ordered array
                    # Support both indexed keys (headline_1) and non-indexed (headline)
                    text_array = []
                    
                    # Collect all headlines (indexed or not)
                    for i in range(1, 10):  # Support up to 9 of each type
                        headline = self.text_content.get(f'headline_{i}') or (self.text_content.get('headline') if i == 1 else None)
                        if headline and headline not in text_array:
                            text_array.append(headline)
