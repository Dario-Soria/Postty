...
function buildGenerationPrompt(params: {
  ...
  if (params.textContent && params.textContent.length > 0) {
    ...
    // VERSION 2: Use the agent's complete prompt directly
    // The agent builds the full prompt in _build_nanobanana_prompt() with === TEXT === section
    // No need for template reader - the prompt already includes all text instructions
    basePrompt = params.userIntent || 'Professional product photography with elegant composition';
    
    // Append text elements info for additional clarity to Gemini
    ...
    logger.info(`📝 Using agent-provided prompt directly (includes text instructions)`);
