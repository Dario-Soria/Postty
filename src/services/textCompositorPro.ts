...
if (fs.existsSync(FONTS_DIR)) {
  ...
  fontFiles.forEach(font => {
    try {
      ...
      const familyName = font.replace(/\.(ttf|otf)$/, '');
      
      // Detectar weight y style del nombre del archivo
      ...
      logger.info(`✓ Font registered: ${familyName}`);
    } catch (e) {
      logger.warn(`Could not register font: ${font} - ${e}`);
    }
  });
}
